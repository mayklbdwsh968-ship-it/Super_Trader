"""
ملف: Manager.py
الوظيفة: نسخة "Railway" من Manager.py — مطابقة تماماً لمنطق التداول
         والتوافق مع العقد الجديد لـ Entry_Decision_File.py و
         Exit_Decision_File.py، بفرقين اثنين عن النسخة المحلية/الأصلية:

         1) الإعدادات الأربعة (Symbol, Secret_Key, Access_Key, Buy_Volume)
            لم تعد تُقرأ من ملف Settings.py، بل تُقرأ مباشرة من متغيرات
            بيئة الخادم (Railway Environment Variables) بنفس الأسماء
            تماماً — لا حاجة لرفع Settings.py أو أي مفاتيح داخل الكود.

         2) نظام التسجيل المركزي (Loger.py) مُعطَّل بالكامل في هذا الملف؛
            لا يوجد أي استيراد له. كل رسائل print() العادية تُطبع مباشرة
            إلى stdout، ويتكفّل Railway بالتقاطها وعرضها/حفظها ضمن
            Logs الخاصة بالخدمة — فلا حاجة لأي نظام تسجيل محلي منفصل.
...
# أسماء متغيرات البيئة المطلوبة على الخادم (Railway) — بنفس الأسماء
# التي كانت موجودة سابقاً كخصائص داخل Settings.py
_REQUIRED_ENV_VARS = ["Symbol", "Secret_Key", "Access_Key", "Buy_Volume"]
...
# ══════════════════════════════════════════════
#   دالة تحميل الإعدادات من متغيرات البيئة (Railway)
# ══════════════════════════════════════════════
def _load_settings_from_env():
    missing = [name for name in _REQUIRED_ENV_VARS if not os.environ.get(name)]
    if missing:
        print(
            "❌ متغيرات بيئة ناقصة على الخادم: "
            f"{', '.join(missing)} — تأكد من ضبطها جميعاً ضمن "
            "Environment Variables في Railway بنفس هذه الأسماء تماماً."
        )
        sys.exit(1)
    ...
    raw_buy_volume = os.environ["Buy_Volume"]
    try:
        Buy_Volume = float(raw_buy_volume)
    except ValueError:
        print(
            f"❌ قيمة متغير البيئة Buy_Volume غير صالحة كرقم: "
            f"'{raw_buy_volume}' — يجب أن تكون رقماً عشرياً (مثال: 0.1)."
        )
        sys.exit(1)

    return Symbol, Secret_Key, Access_Key, Buy_Volume
...
def main():
    ...
    # ─────────────────────────────────────────
    # تحميل الإعدادات من متغيرات بيئة الخادم (Railway)
    # ─────────────────────────────────────────
    Symbol, Secret_Key, Access_Key, Buy_Volume = _load_settings_from_env()
    # ملاحظة: TP لم يعد رقماً ثابتاً — أصبح SL/TP1/TP2 ديناميكيين
    # بالكامل، يُبنيان عند الدخول عبر Init_Position_Risk_Management
