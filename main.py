...
# ══════════════════════════════════════════════
#   خادم فحص الصحة (Health Check) لأجل الاستضافة على Koyeb
#   لا علاقة له بمنطق البوت إطلاقاً — فقط يرد 200 OK لأي طلب HTTP
# ══════════════════════════════════════════════
class _HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        pass  # تعطيل طباعة سجلات HTTP في الطرفية
...
def _run_health_check_server():
    ...
    try:
        ...
        logger.warning(f"⚠️ تعذّر تشغيل خادم فحص الصحة: {e}")
...
def main():
    ...
    Thread(target=_run_health_check_server, daemon=True).start()
